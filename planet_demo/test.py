...
class Tractor_beam(object):
    def __init__(self,sprite,player,screen):
        self.sprite = sprite
        self.player = player
        self.pos = [0,0]
        self.sprite2 = self.sprite
        self.centerPos = [0,0]
    def draw(self):
        pressed = pygame.mouse.get_pressed()
        if pressed[2]:
    ...
    def update(self):
        X, Y = pygame.mouse.get_pos()
        self.rotation = -math.atan2(Y - self.player.pos[1], X - self.player.pos[0]) * (180 / 3.14)
        self.centerPos[0] = self.player.pos[0]- self.sprite.get_rect().width // 2
        self.centerPos[1] = self.player.pos[1]- self.sprite.get_rect().height // 2
        self.draw()
        self.sprite = pygame.transform.rotate(self.sprite2, self.rotation)
...
if __name__ == "__main__":
    ...
    player = Player(spritesheet.subsurface(source_rects["jet"]), screen, [(screenWidth / 2) - 25, screenHeight / 2],
                    [(0), (0)], [0, 0], [planet])
    tractor = Tractor_beam(spritesheet.subsurface(source_rects["sword"]),player,screen)
